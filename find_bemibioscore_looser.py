...
for conv in conversations:
    ...
    with open(log_path, 'r', encoding='utf-8') as f:
        for idx, line in enumerate(f):
            if "BemiBiosEntryPoint" in line or "BemiBiosCore.c" in line:
                try:
                    step = json.loads(line)
                except Exception:
                    print(f"  Line {idx}: JSON load error")
                    continue
                
                sidx = step.get("step_index")
                stype = step.get("type")
                # Look for tool name or arguments
                tool_info = ""
                if "tool_calls" in step:
                    tcs = []
                    for tc in step["tool_calls"]:
                        tcs.append(tc.get("name"))
                    tool_info = f" | tool_calls: {tcs}"
                
                print(f"  Step {sidx or idx} | Type: {stype}{tool_info} | Length: {len(line)}")
