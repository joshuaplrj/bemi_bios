...
| Macro fusion Jcc never matches | `MacroOpFusion.c` | High | **FIXED** — 3-opcode parameter |
| MSR read-only logic inverted | `MsrShadow.c` | Medium | **FIXED** — blocks writes with EFI_ACCESS_DENIED |
| SMM state never saved/restored | `SmmHandler.c` | High | **FIXED** — real save buffer + TSC latency measurement |

## Remaining Phases

### Phase 4: HW Compat Completion

| # | Task | Detail | Priority |
|---|---|---|---|
| 4.1 | Complete CPUID leaves | 0x00-0x1F, 0x4000_0000-0x4FFF_FFFF, 0x8000_0000-0x8000_001F | High |
| 4.2 | Expand MSR shadow | MTRRs, PAT, debug MSRs, perf counters | Medium |
| 4.3 | Connect APIC to interrupt path | Shadow writes → VMCS entry-interruption injection | High |

### Phase 5: DBT Pipeline Rewrite
