...
extern EFI_STATUS BemiVmxExitHandler(VM_EXIT_INFO *ExitInfo, GUEST_STATE *GuestState);
...
extern EFI_STATUS BemiSvmExitHandler(VM_EXIT_INFO *ExitInfo, GUEST_STATE *GuestState);
  ...
  )
{
  ...
  if (vendor == HypervisorIntel) {
    gHypervisorBackend.Init = VmxInit;
    gHypervisorBackend.VcpuCreate = VmxCreateVcpu;
    gHypervisorBackend.VcpuRun = VmxRunVcpu;
    gHypervisorBackend.HandleExit = BemiVmxExitHandler;
  } else if (vendor == HypervisorAmd) {
    gHypervisorBackend.Init = SvmInit;
    gHypervisorBackend.VcpuCreate = SvmCreateVcpu;
    gHypervisorBackend.VcpuRun = SvmRunVcpu;
    gHypervisorBackend.HandleExit = BemiSvmExitHandler;
  } else {
    return EFI_UNSUPPORTED;
  }

  status = gHypervisorBackend.Init(CpuCount);
  return status;
}
