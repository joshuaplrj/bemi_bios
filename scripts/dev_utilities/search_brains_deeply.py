    ...
    try:
        with open(log_path, 'r', encoding='utf-8', errors='ignore') as f:
            for idx, line in enumerate(f):
                # Simple check first
                if not any(tf in line.lower() for tf in target_files):
                    continue
                ...
                for tc in tool_calls:
                    ...
                    if tname in ["write_to_file", "replace_file_content", "multi_replace_file_content"]:
                        ...
                        if isinstance(args, dict):
                            ...
                            if tf_path and code:
                                basename = os.path.basename(tf_path).lower()
                                if basename in best_versions:
                                    ...
                                    # Update logic:
                                    # 1. If current is not truncated and best is truncated -> update
                                    # 2. If truncation status is same, and current is longer -> update
